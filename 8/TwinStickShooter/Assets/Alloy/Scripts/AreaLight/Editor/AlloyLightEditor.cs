...
public class AlloyLightEditor : Editor
{
    ...
    private void ColorIntensityField() {
        if (!m_hasAreaComponent) {
            ...
            var intensity = m_areaEditor.serializedObject.FindProperty("m_intensity");
            ...
            //    var lumens = AlloyUtils.IntensityToLumens(intensity.floatValue);
            //    var newLumens = EditorGUILayout.FloatField("Lumens", lumens);
                
            //    if (!Mathf.Approximately(lumens, newLumens))
            //        intensity.floatValue = AlloyUtils.LumensToIntensity(newLumens);
            //}

            if (EditorGUI.EndChangeCheck()) {
                intensity.floatValue = Mathf.Max(0.0f, intensity.floatValue);
            }
        }
    }
    ...
    private void ColorAreaField() {
        if (m_hasAreaComponent) {
            var prop = m_areaEditor.serializedObject.FindProperty("m_color");

            prop.colorValue = EditorGUILayout.ColorField(new GUIContent("Color"), prop.colorValue, true, false, false, null);
        }
        else {
