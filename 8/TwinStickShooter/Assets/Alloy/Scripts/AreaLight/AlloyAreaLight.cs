...
[ExecuteInEditMode]
[AddComponentMenu(AlloyUtils.ComponentsPath + "Area Light")]
public class AlloyAreaLight : MonoBehaviour {
    ...
    [SerializeField]
    private bool m_isAnimatedByClip = false;
    ...
    public bool IsAnimatedByClip {
        get { return m_isAnimatedByClip; }
        set { m_isAnimatedByClip = value; }
    }
    ...
    public void UpdateBinding() {
        ...
        if (lightCache.type == LightType.Directional) {
            ...
            col.a = Mathf.Ceil(lengthWeight * 1000.0f) + Mathf.Min(radiusWeight, 0.999f);
        }
    ...
    private void Reset() {
        ...
        if (l != null) {
            // Match intensity to Unity's post-linear multiply behavior.
            m_intensity = Mathf.LinearToGammaSpace(l.intensity);
            m_color.r = l.color.r;
            m_color.g = l.color.g;
            m_color.b = l.color.b;
        } else {
    ...
    private void Update() {
        if (m_isAnimatedByClip
            || Light.range != m_lastRange) {
            UpdateBinding();
        }
    }
}
